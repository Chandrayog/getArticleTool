...
def search_googleScholar(query, headers, _gs_pages, records, _title, _keyword, _abstract, scrpr_api, _from_yr, _to_yr_,
                         logging_flag, data):
    rec = 0
    if _title:
        # request url
            ...
            for item in soup.select('[data-lid]'):
                try:
                    ...
                    resp_obj = {"entities": {"Search Engine": "Google Scholar",
                                             "Attributes found": "Title, URLs, Authors, Cited count, Type, Published "
                                                                 "date, Abstract",
                                             "items": [
        ...
        if _from_yr:
            ...
            for i in tqdm(range(1)):
                ...
                if response.status_code != 200:
                    ...
                    logger.writeError("Logging Error:" + str(response.status_code), None, _engine, logging_flag)
                    ...
                    if int(pages) > 100:
                        print(
                            ...
                            "Total No of total records found :",
                        ...
                        for i in range(pages):
                            ...
                            for item in soup.select('[data-lid]'):
                                try:
                                    ...
                                    # response object
                                    resp_obj = {"entities": {"Search Engine": "Google Scholar",
                                                             "Attributes found": "Title, URLs, Authors, Cited count, "
                                                                                 "Type, Published date, Abstract",
                                                             "items": [
                                                                 {"DOI": str(['No information found']),
                                                                  "Title": item.select('h3')[0].get_text(),
                                                                  "URLs": item.select('a')[0]['href'],
                                                                  "Authors": re.sub("[^A-Za-z]", " ", str(
                                                                      item.select('.gs_a')[0].get_text()).split('-', 1)[
                                                                      0]),
                                                                  "Publication Name": str(['No information found']),
                                                                  "ISSN": str(['No information found']),
                                                                  "Cited count": cc,
                        ...
                        for i in range(pages):
                            ...
                            if response.status_code != 200:
                                ...
                                logger.writeError("Logging Erorr:" + str(response.status_code), None, _engine,
                                                  logging_flag)
                                ...
                                for item in soup.select('[data-lid]'):
                                    try:
                                        ...
                                        resp_obj = {"entities": {"Search Engine": "Google Scholar",
                                                                 "Attributes found": "Title, URLs, Authors, Cited "
                                                                 ...
                                                                 "items": [
                                                                     {"DOI": str(['No information found']),
                                                                      ...
                                                                      "URLs": item.select('a')[0]['href'],
                                                                      "Authors": re.sub("[^A-Za-z]", " ", str(
                                                                          item.select('.gs_a')[0].get_text()).split('-',
                                                                                                                    1)[
                                                                          0]),
                                                                      "Publication Name": str(['No information found']),
                                                                      "ISSN": str(['No information found']),
                ...
                for i in range(pages):
                    ...
                    for item in soup.select('[data-lid]'):
                        try:
                            ...
                            resp_obj = {"entities": {"Search Engine": "Google Scholar",
                                                     "Attributes found": "Title, URLs, Authors, Cited count, Type, "
                                                     ...
                                                     "items": [
                                                         {"DOI": str(['No information found']),
                                                          ...
                                                          "URLs": item.select('a')[0]['href'],
                                                          "Authors": re.sub("[^A-Za-z]", " ", str(
                                                              item.select('.gs_a')[0].get_text()).split('-', 1)[0]),
                                                          "Publication Name": str(['No information found']),
                                                          "ISSN": str(['No information found']),
                                                          "Cited count": cc,
                                                          "Affiliation": str(['No information found']),
